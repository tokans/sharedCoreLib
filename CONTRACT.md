...
| `sharedcorelib/masters` | merge: `mergeMasterOptions`, `pickMode`, `DROPDOWN_MAX`, `MasterOption`; verify: `verifyManifestSignature`, `sha256Hex`, `decryptTransport`, `meetsMinVersion`, `verifyAndDecryptManifest`, `genericManifestSchema`; OTA: `createOtaUpdater(config)`; **common masters + namespacing**: `getCommonBaked`, `COMMON_MASTER_IDS`, `isCommonMaster`, `COMMON_SCOPE`, `qualifyMasterKey`, `parseMasterKey` (see §5.7). **City seed is LAZY (bundle-size):** the 600KB+ `cities.seed.json` loads only via dynamic import (`sharedcorelib/masters/cities` → `loadCommonCities()`/`getBakedCities(parent)`, also re-exported from `/masters`) so it lands in its own async chunk; `getCommonBaked("city")` returns [] until `loadCommonCities()` resolves once — **prewarm before a city picker** (or use async `getBakedCities`). Country/currency/relationship stay synchronous. | release `baseUrl`, signing `pubkeyHex`, `transportKeyB64`, its OWN **app-specific** master registry + zod schemas (reusing the common sets), `getLastRevision`/`applyEntry` adapters, and (for L2) `cacheDir` + `cacheNamespace` |
...
| `sharedcorelib/docintake` | **Document intake** — what a file IS and how to open it, for every password-protected import (bank statement, tax document, broker statement): `createDocIntake(config)` → `open(bytes, filename, passwordCandidates, log)` → `IntakeResult` (`extraction` + `passwordUsed` + `sourceKind` + `log`); magic-byte `detectFileKind` (extension is only the tiebreaker — an `.xlsx` is itself a zip, and an encrypted `.xlsx` and a plain legacy `.xls` are both OLE/CFB); zip-entry extraction with re-detection of the inner file; `DocumentPasswordRequiredError` / `NativeCapabilityError` / `UnsupportedDocumentError`; `createParseLog`/`ParseLog`. Emits an `Extraction` (`pdf` rows \| `grid` sheets \| `text`) — the input `@scandoc/core/docmodel`'s `buildDocModel` structures. The two packages compose WITHOUT depending on each other: `Extraction` is declared structurally on both sides. | the native seams as DI: `parsePdf` (PDFium via the app's Tauri command), `extractZip`, optional `xlsx`/`ooxmlCrypto` overrides. **Omit a seam and that branch refuses up front with `NativeCapabilityError`** — which is how an app gates PDF/ZIP on mobile while keeping the pure-JS xlsx path working. Structuring options (number/date recognizers, strip patterns) belong to `@scandoc/core/docmodel`, not here. |
...
| `sharedcorelib/ui` | **Purge-safe:** `cn`, `ClassValue` — also exported from the granular **`sharedcorelib/ui/cn`** subpath so an app's app-wide `cn` import does NOT drag in the styled kit (SuiteShell/Sheet/BackupPanel/radix/lucide); prefer `sharedcorelib/ui/cn` for `cn`. **publisher attribution** (`SupportedByTokans`, `tokansAttribution`, `SUPPORTED_BY_LABEL`, `TOKANS_URL`, `TOKANS_LOGO_DATA_URI`); **`AppHarness`** unstyled responsive primitive (slots + `pickOrientation`/`useViewportWidth`/`themeStyle`/`DEFAULT_THEME`/`SuiteThemeToken`/`chromeActions`). **Styled kit (needs §4.2 preset + `theme.css` + content glob):** **`SuiteShell`** (`SuiteNavItem`/`SuiteAction`/`SuiteAccount` — sidebar + mobile top bar + 3-button bar + adaptive central sheet + More drawer + `profile` slot + tier-gated `account` + ADDITIVE `userSwitch?: SuiteUserSwitch` — the paid multi-user switcher, **paid-gated by construction**: renders NOTHING unless the prop is provided AND `members.length > 1`, so free single-user chrome stays byte-identical (invariant 3); member management itself is myLifeAssistant-only) and **`Sheet`**/`SheetContent`/`SheetClose` drawer. Also: **`sharedcorelib/tailwind-preset`** (Tailwind preset) + **`sharedcorelib/ui/theme.css`** (default token values). **Dev/test overrides (DEV-ONLY, no real entitlement/identity):** `createTierOverride`/`TestTierChooser` (tier preview; pure resolver in `sharedcorelib/tiers`) and `DevPill`/`DevRoleSwitcher`/`DevIdentitySwitcher`/`parseDevTestIdentities` (role + backend test-identity switchers, promoted out of myWorkAssistant) — dumb UI over an app-injected callback; render behind the app's own dev-build check. **`DevFloatingPanel`** is the bottom-right floating "dev tools" affordance `TestTierChooser` renders internally, generalized to hold arbitrary children (e.g. several role/identity sections at once behind ONE button) — self-gates on its `dev` prop, so it's safe to mount unconditionally at the app-shell root instead of behind a Settings page. | for `SuiteShell`: `brand`, `nav` (precomputed `state`), `centralActions`, `actions`, `profile` slot, optional `account` (tier ≥ 2), optional `userSwitch` (`{current, members, onSwitch}`, paid apps only), `onExternal`. Must render inside the app's `react-router-dom` Router. **primitive** kit (`FiniteSetInput`, shadcn primitives) still deferred — see §4. For the dev switchers: the role catalog / test identities + an `onToggle`/`onSwitch` callback that applies the choice (e.g. into a local UAM store, or a real session-token swap) |
...
| `sharedcorelib/multiuser` | `generateSharedKey`, `multiWrap`/`addMember`/`removeMember` (rotate-on-remove), `unwrapShared`, `coUserRewrap`; compartments — send-side scoping: `privateCompartment`/`compartmentOf`/`canAccessCompartment`/`syncTargets`/`rowsForRecipient`; **crypto-hard per-user compartment keys**: `createPrivateCompartmentKey(member)` (256-bit PK wrapped ONLY under the owner's user key), `unwrapPrivateCompartmentKey`, `keyForCompartment(compartment, ring)` (→ key for `shared`/own-private, **null** for another member's private), `sealForCompartment`/`openForCompartment` (AAD-bound to the compartment), `CompartmentKeyring`/`WrappedCompartmentKey` — a `private:<userId>` payload seals under that user's PK, unreadable by other members even with the ciphertext (the sync filter is defense-in-depth, NOT the boundary); **member-class policy (UI-soft ONLY — no confidentiality claim; crypto-hard stays in compartments)**: `createMemberClassPolicy(rules)` / `createChildSoftPolicy(extraRules?)` → `MemberClassPolicy.isFeatureAllowed(memberClass, feature, categories?)` (first match wins, no rule ⇒ allowed), `MemberClassRule`, `CHILD_SOFT_DEFAULT_RULES`, `SENSITIVE_FEATURE_CATEGORIES` (`["finance","credentials","estate"]` — an OPEN vocabulary, apps tag their own gates). **Contract:** `contracts/multiuser-activation.md` | per-user keys; row `compartment` tags; the app's policy rules + per-gate category tags. Reuses the recovery seal. **Member-management UI is myLifeAssistant-only; free single-user behavior is unchanged when nothing is configured** |
...
| `sharedcorelib/subscription` | The **ONE suite subscription** (decision 20) — single paid entitlement across every app; **no Patron/Partner**, no per-product sub, no cross-discount. THREE pricing tiers `essential\|plus\|max` (`SuiteTierId`/`SUITE_TIERS`/`SUITE_TIER_IDS`/`tierRank`/`tierAtLeast`/`nextTier`); **all features ship at tier 1** — tiers differ only on 3 cost-linked axes: (1) cloud-token allowance metering (`TokenUsage`/`remainingCloudTokens`/`recordCloudUsage`/`applyTopUp`/`cloudTurnDecision` → explicit at-cap choices, BYO uncapped; `cloudLegAllowed`), (2) **model access** (`ModelAccessClass` `own-hosted\|third-party-economy\|frontier-latest`; `allowedModelClasses`/`isModelClassAllowed`/`assertModelClassAllowed` — `max` unlocks frontier-latest), (3) **marketplace earning** (`marketplacePublishMode`/`canEarnOnMarketplace` — essential = freeware-only, plus/max monetizable). Entitlement: `subscriptionActive`/`suiteEntitlement`/`createSuiteEntitlement`/`SuiteEntitlement`/`SUITE_UNLOCKS`. Persistence: **`common#SuiteSubscription`** shared single-row table (`SUITE_SUBSCRIPTION_SCHEMA`, `owner:"common"`, `shared:true`) + `createSubscriptionStore(db)` (`get`/`set`/`clear`/`ensure`) — ONE row read by all apps. Pure/DI; mints nothing, no crypto. | an injected `SqlDb` to read the shared row; the billing layer (myLifeAssistant checkout) writes it; injected `nowIso` for active/metering checks; a `PricingPolicy` (price tracks cost) for quotes |
