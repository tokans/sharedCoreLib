...
| `sharedcorelib/ui` | `cn`, `ClassValue`; **publisher attribution**: `SupportedByTokans` (renders the baked Tokans mark + wording, links to `www.tokans.org`), `tokansAttribution`, `SUPPORTED_BY_LABEL`, `TOKANS_URL`, `TOKANS_LOGO_DATA_URI` | the status-bar `className` (and, in Tauri, an `onActivate` OS opener so the link opens in the real browser); heavier UI kit deferred — see §4 |
